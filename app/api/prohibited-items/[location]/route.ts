...
const PROHIBITED_URL = process.env.NEXT_PUBLIC_CONTENT_URL!;
const httpsAgent =
...
export async function GET(
  ...
  console.log("→ proxying prohibited-items with auth:", auth);

  try {
    const res = await axios.get(
      `${PROHIBITED_URL}/api/prohibited-items/${encodeURIComponent(location)}`,
      { httpsAgent, withCredentials: true, headers: { Authorization: auth } }
    );
    return NextResponse.json(res.data, { status: res.status });
  } catch (e) {
    return handleError("GET /prohibited-items", e as AxiosError);
  }
}
