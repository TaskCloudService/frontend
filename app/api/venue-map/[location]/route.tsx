...
const VENUE_MAP_URL = process.env.NEXT_PUBLIC_CONTENT_URL!; 
const httpsAgent =
...
export async function GET(
  ...
  console.log("→ proxying venue-map with auth:", auth);

  try {
    const res = await axios.get(
      `${VENUE_MAP_URL}/api/venue-map/${encodeURIComponent(location)}`,
      { httpsAgent, withCredentials: true, headers: { Authorization: auth } }
    );
    return NextResponse.json(res.data, { status: res.status });
  } catch (e) {
    return handleError("GET /venue-map", e as AxiosError);
  }
}
